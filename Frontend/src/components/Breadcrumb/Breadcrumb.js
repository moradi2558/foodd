...
const Breadcrumb = ({ links }) => {
  return (
    <div className="breadcrumb_container tw-relative tw-flex tw-items-center tw-justify-center tw-pt-20 tw-pb-28">
      <div className="tw-flex tw-flex-col tw-justify-center tw-items-center">
        <h2 className="tw-text-5xl tw-mb-10">
          {links[links.length - 1].title}
        </h2>
        <div className="tw-flex tw-items-center tw-flex-row-reverse">
          {links.map((link) =>
            link.id === links.length ? (
              <span key={link.id} className="tw-text-yellow-500">
                {link.name ? link.name : link.title}
              </span>
            ) : (
              <div
                className="tw-flex tw-flex-row-reverse tw-items-center"
                key={link.id}
              >
                <Link to={`/${link.to}`} className="tw-text-gray-400">
                  {link.name ? link.name : link.title}
                </Link>

                <IoIosArrowBack className="tw-text-gray-400" />
              </div>
            )
          )}
        </div>
      ...
      <div className="tw-absolute tw-w-full tw--bottom-1 lg:tw--bottom-6 tw-rotate-180">
        <svg
